...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (username.trim().length < 2) return toast.error("Nama terlalu pendek!");
    
    login(username);
    toast.success("Berhasil Masuk");
    navigate("/"); // Menuju Dashboard
  };

  return (
    // Background Biru Full Screen
    <div className="min-h-screen flex items-center justify-center bg-blue-600 p-6">
      {/* Boks Lebih Kecil (max-w-xs) */}
      <Card className="w-full max-w-xs rounded-3xl shadow-2xl border-none bg-white">
        <CardContent className="p-8 space-y-6 text-center">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <Input
              ...
              className="text-center font-bold h-12 rounded-xl border-slate-200"
            />
            <Button type="submit" className="w-full h-12 bg-blue-600 hover:bg-blue-700 font-black rounded-xl shadow-lg">
              MASUK
            </Button>
          </form>
        </CardContent>
